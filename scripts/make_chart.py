...
/* Clan is not colour-coded: it renders as text like every other field. */
.clan{color:inherit}
...
/* Each person is a chip. Direct children only -- a cross-reference row is also
   a .reg-rel, but its links sit inside an <em> of running prose ("For second
   wife and offspring see below, 76, 90-3") and must stay prose, not become
   buttons mid-sentence. The border is the affordance here, so the register's
   dotted underline is dropped rather than drawn on top of a box; hover moves
   the border and the fill, which is what the rest of the chrome does. */
...
function openCard(a){
  ...
  /* Three edits to the clone, none of them touching rel_row -- the register
     ...
     Rows with no .rel-l are the cross-reference rows -- running prose, which
     keeps its own punctuation and its inline links. Everything inside a
     labelled row is rel_link output, so every .num in one is a relation. */
